...
Related references:
\begin{itemize}
%\item{\url{http://lsst-desc.org/sites/default/files/LSST_DESC_Publication_Policy_v6_15aug2016.pdf}{DESC Publication Policy}}
\item{\FIXME{Link to http://lsst-desc.org/sites/default/files/LSST\_DESC\_Publication\_Policy\_v6\_15aug2016.pdf will not compile because of the hyphen in the URL}}
\item{\FIXME{Link to DESC Author Guide}}

\item{\url{https://www.aanda.org/doc_journal/instructions/aadoc.pdf}{Astronomy \& Astrophysics author guide}}

\item{\url{http://journals.aas.org/authors/manuscript.html}{Astrophysical Journal author guide}}

\end{itemize}
%\url{http://www.aanda.org/doc_journal/instructions/aa_instructions.pdf}.

A\&A also has a useful English usage guide, available at 
...
The \verb|start\_paper| package \COMMENT{SD: Is package the right term?} is available to facilitate DESC paper development, in LaTeX and in GitHub...
...
\COMMENT{SD: Is DESC\_macros the same as lsstdesc\_macros?}
...
\COMMENT{SD: Not sure what to say about it}

\section{HOW TO REFER TO DESC AND LSST} 

\begin{itemize}
\item{DESC should be spelled out at the first use (in both the abstract and the main text of the paper) and a footnote added with the URL for the DESC Web site, \verb|\footnote{\url{http://lsst-desc.org}}|. \FIXME{This does not currently work because of the hyphen in the URL}.  
\COMMENT{SD: Another option might be to refer to the DESC white paper \url{http://adsabs.harvard.edu/abs/2012arXiv1211.0310L}, although this is dated.  I had thought that the SRM might be on the arXiv, but it does not seem to be.}}
...
Do not write LSST's or DESC's.  Instead write, e.g., the weak lensing pipeline for DESC.
...
Guidelines for defining author lists and the text for the Acknowledgments section, including contribution statements, are presented in the DESC Author Guide.  The Publication Policy is fairly prescriptive on each of these topics.  \FIXME{Add link to the Author Guide}
...
defined in {\tt DESC\_macros.tex}.  Non-integer values should place the symbol over the
decimal point.  Spacing is a bit tricky because of the different shapes of the
digits 0--9, but the commands \verb|\pdeg|, \verb|\parcm|, and \verb|\parcs| in {\tt DESC\_macros.tex}
...
spaced as well.
\FIXME{AM: Inconsistency with actual lsstdesc\_macros.} 
...
\COMMENT{SD: So do we remove this recommendation or try to enumerate the conventions by journal?}
...
``$\backslash$la'' (and ``$\backslash$ga'') or the {\tt DESC\_macros.tex} macro
...
\item
Use the La\TeX~bibsty bibliography style file provided by the journal, and/or refer to the bibliography guidelines of the journal.  For several journals, these are also available in \verb|start\_paper|.  For \FIXME{name the journals} the \verb|start\_paper| versions of the bibsty files have been modified so that arXiv numbers are displayed instead of the egregious ``Arxiv e-prints'' \FIXME{Can we do this?}; the bibsty files have also been \FIXME{will be} modified to include hyperlinks to ADS, if they did not already.  

\item
The   \verb|DESC\_tex| GitHub repository includes DESC\_bib, which contains a bibliography of recommended and commonly cited references in DESC papers.  These will include LSST and DESC product papers.  \FIXME{None yet}
...
\FIXME{SD: Do we need this?  Or can we make more specific recommendations regarding the color map?}
...
\COMMENT{SD: I think that we'll need to establish the convention with the first such paper, then update the Style Guide with it.}
...
{http://pdg.lbl.gov/2013/reviews/rpp2013-rev-rpp-intro.pdf} \FIXME{Make a hyperlink},
...
\COMMENT{SD:  ApJ at least uses nodata for empty entries in tables.}

...
\verb|DESC_macros.tex|, available {\bf somewhere}.  To use, insert the line
\verb|\input{DESC_macros}| after the initial \verb|\documentclass| command in your input
file.

\COMMENT{AM: We eventually need to merge in the real lsstdesc\_macros file, if we intend to demo it (though I'm not sure we need to). I've briefly and inconclusively discussed with Phil the best way of distributing DESC macros and styles (e.g.\  the note style), but details are tbd.}
...
most easily with the \verb|\dots| command defined in Planck.tex, which works in

...
Pay particular attention to the size of characters (Sect.~17.3.3) and
...
\item {\it Size\/}.  A\&A prints figures in three widths,
88\,mm, 120\,mm, and up to 
180\,mm. {\bf Each figure should be designed for its final size, whether
single-column, side caption, or two-column.} Simply rescaling a figure
...
In some cases a ``full width'' figure will fit on the page better or
...
\item {\it Lettering\/}.  There are differing opinions about whether labels
...
8\,pt\footnote{In the digital publishing world,
...
\item {\it Legends\/}.  A legend within the plot identifying colours and
...
\begin{itemize}
\item The transfer function/colour map used in figure~15 of {\it Planck 2013
results.~I\/}.  This is suitable for maps with low dynamic range and zero mean
(e.g., the CMB).

\item The transfer function/colour map used in figures~9 and 10 of
...
\item {\it Consistency of style and colour across figures\/}.  Use a uniform
...
\item {\it Time axes in days\/}.  Time should be plotted in days rather than
...
\item {\it SEDs\/}.  SEDs should be plotted with frequency increasing to the
...
\item {\it Geometric distortion\/}.  {\bf Do not} scale the height and width of a figure independently to make it fit a spot in a paper.  The geometrical distortion introduced is unacceptable.  The consequences are particularly undesirable for Mollweide projections, whose geometrical characteristics are ruined.
\end{enumerate}
...
\item In Sect.~15, item~4 we emphasized that minus signs can only be typeset in
...
single-column format we use here.  The \verb|\caption| command from A\&A La\TeX\
used in the \verb|PlanckTable.tex| template file produces results in the standard

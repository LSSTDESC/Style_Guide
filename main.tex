...
\newcommand*{\version}{Version 1.1.0 (2020-04-16)} %{Version \today}
...
\fancyhead[L]{}
\fancyhead[R]{
  \centering{\includegraphics[width=0.95\textwidth,angle=0]{figures/header}}
}
\fancyfoot{}
\pagestyle{fancy}
...
\item{The first reference to the observatory by itself should be to ``Vera C. Rubin Observatory''.  Subsequent references should be either ``Rubin Observatory'' or ``Rubin'' (not VRO).}

\item{The telescope should be referred to as the ``Simonyi Survey Telescope at Rubin Observatory''.}

\item{The camera should be referred to as ``Rubin Observatory LSST Camera'' at the first reference.  Subsequently, use ``LSST Camera''.}

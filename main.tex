...
\FIXME{AM: Link to individual journal author instructions from here. Also link to the publication policy and author guide.}
...
The start\_paper package is available to facilitate DESC paper development, in LaTeX and in GitHub...

\subsection{DESC\_macros.tex}

La\TeX\ commands useful and specific to DESC papers are found in
\verb|lsstdesc_macros.sty|, available as part of the \href{https://github.com/LSSTDESC/desc-tex}{\tt desc-tex} GitHub project, or directly downloadable \href{https://raw.githubusercontent.com/LSSTDESC/desc-tex/master/styles/lsstdesc_macros.sty?token=AKdGNisspyfcn5HL4yoV16o5vQh58v13ks5ZVaazwA%3D%3D}{here}.  To use, insert the line
\begin{verbatim}
...
\COMMENT{AM: Add note about the `draft' option to hyperref.}


\section{HOW TO REFER TO DESC AND LSST \label{sec:lsst_refs}}

Some advice goes here about how to refer to DESC, LSST, the LSST Project, and the LSST Camera Project, including recommended references.

Footnote a URL for the DESC Web site?

LSST should be spelled out at the first use (in both the abstract and the main text of the paper).

\COMMENT{AM: We may eventually need a standardized of referring to data releases, analogous to the different Planck surveys. General comments about when other experiments get italicized would be good, if they don't vary too much among journals (or just pointing out that they do would be good).}

\subsection{The possessive form}

Do not write LSST's or DESC's.

\section{DATES}

Use Year Month Day format, UTC.

Advice about hours-minutes-seconds?   
...
\FIXME{AM: The removed ``Title'' section must be resurrected, if only for Key papers. At least, I assume there will be some standard titling of them.}
...
{\bf SD:  Is there an ApJ-specific list of keywords, or are they standardized?}
\COMMENT{AM: Each journal seems to have its own keyword list, although, in practice, they may actually be standardized.}

\FIXME{AM: ``Authors and Acknowledgments'' should probably exist here, even if it's just a pointer to the author guide.}
...
common units are defined in \verb|lsstdesc_macros.sty| so that they can be used in or out of
...
% digits 0--9, but the commands \verb|\pdeg|, \verb|\parcm|, and \verb|\parcs| in DESC\_macros.tex
...
% spaced as well.
% \FIXME{AM: Inconsistency with actual lsstdesc\_macros.} \COMMENT{AM: I believe that $2.8''$-type formatting is also generally acceptable, not to mention less absurd-looking, as long as one format or the other is used consistently.}
\item \FIXME{Removed item about placement of \degree, $'$, $''$ symbols wrt decimal points, as it used macros we don't have.}
...
See Sect.~\ref{sec:tables} for more details.
...
\COMMENT{AM: Seems like a strange place for this note.}
...
being referred to and those in the paper being written.   {\bf SD:  Is this A\&A specific?}
...
e.g., {\tt HEALPix}. \COMMENT{AM: journal-specific}
%, {\tt PLIK}, and {\tt Commander} (obtained with
...
\COMMENT{AM: note differing uses of emph and it}
...
\COMMENT{AM: \ldots except that multiple references to the same authors in a list get grouped together, right? As in (Digel et~al. 2001; Mantz et~al. 2003, 2010; Peiris et~al. 2006).}
...
Refer (once)
to the relevant LSST or DESC product papers (see Sect.~\ref{sec:lsst_refs}), and summarize as briefly as
...
\COMMENT{AM: Prohibition on minus signs in this context.}

\item For the plural of ``halo'' write ``halos,'' not ``haloes.''
\COMMENT{AM: Might fall afoul of British spelling, when required.}
...
simply remove the brackets altogether.  {\bf SD:  I'm not getting this one}
\COMMENT{AM: Agreed; journal styles differ on this anyway.}
...
\item
The use of British or American spelling depends on the journal.  Table {\bf reference the table} summarizes the differences.
...
``$\backslash$la'' (and ``$\backslash$ga'') or the DESC\_macros.tex macro
...
\COMMENT{AM: `label' should also be the mechanism for referring to figures, tables and sections. Let's make this a separate point from the punctuation note.}
...
Pay particular attention to the size of characters and
extraneous white space around figures.
\medskip
...
figure shows \dots''. {\bf SD:  I agree with the last part but not the first}

\COMMENT{AM: I think we should keep some of the exhortions that appeared in the original. (1) Use vector graphics whenever feasible. (2) Produce graphics of (or close to) the size they will have in the paper. We might also encourage people to produce versions of figures with extra-large labels for use in talks. In addition, (3) captions should be descriptive enough that the basic content and message of a figure are understandable, independent of the main text.}

\subsection{Line plots}

\COMMENT{AM: This section seems to mean ``anything other than images and maps'', not strictly ``line plots''.}

\begin{enumerate}
\item {\it Format\/}. Output should be EPS, PDF, or other vector format
...
($12.9' \pm 1.4'$) would shift the estimates (all up or all down 
...
{Astronomical maps are in general ``pseudocolour'' maps, that is, they map
...
\smallskip\noindent {\it Color map\/}. Choose color values for each color
...
\COMMENT{AM: In some circumstances, we will almost certainly want consistent color schemes used across papers. I'm thinking of cosmology parameter constraint plots in key papers, at least. That might be one application where we do want to provide script(s) to ensure that the key survey results are presented in a uniform-looking way.}
...
\COMMENT{AM: I would advocate something similar to what I said for figures: there should be a caption (or equivalent notes) that make the content understandable without having to refer to the text. I have seen far too many papers where neither the table title, its notes, nor the main text are sufficient to know wtf is being listed in a given column. (ApJ's caption-less table style is particularly bad for this.)}
...
\begin{enumerate}
\item Excessive numbers of digits should not be given.  In general, the number
...
\item Missing values should be indicated by an ellipsis, ``\dots,'' obtained
most easily with the \verb|\dots| command, which works in
both text and math modes.  Don't try to produce an ellipsis with periods (full
stops).  The spacing will be wrong... \COMMENT{AM: I think em-dashes are also sometimes used.}

\item When entries in a column are to be compared, align the decimal points in
both the quantity and the uncertainty. \COMMENT{AM: Also, align $\pm$ symbols when possible.} \FIXME{AM: We can add some pointers re: how to do these things. Some example wouldn't be too hard to dredge up, though nothing as involved as the ones in the Planck guide.}
